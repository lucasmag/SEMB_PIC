...
const unsigned char pixels[70][43] = {
...
unsigned char RobertsCross(short i, short j){
    ...
    if(i == 70 - 1 || j == 43 - 1){
        if(i != 70 - 1){
            media = (pixels[i][j] + pixels[i + 1][j])/2;
        }
        else{
            if(j != 43 - 1){
                media = (pixels[i][j] + pixels[i][j + 1])/2;
            }
            else{
                media = pixels[i][j];
            }
        ...
        media = (pixels[i][j] + pixels[i][j + 1] + pixels[i + 1][j] + pixels[i + 1][j + 1]) / 4;
    Truncar_Pixel(&media);
